...
from backend.models.User_types_model import User_Types_Model
model = User_Types_Model()


user_types_blueprint = Blueprint('/user_types_blueprint', __name__)

# Create a user type
@user_types_blueprint.route('/create_user_type', methods=['POST'])
@cross_origin()
...
@user_types_blueprint.route('/user_type/<ust_id>', methods=['POST'])
@cross_origin()
...
# List all user types
@user_types_blueprint.route('/user_types', methods=['POST'])
@cross_origin()
def user_types():
    return jsonify(model.user_types())

# Update usertype by ID
@user_types_blueprint.route('/update_user_type/<ust_id>', methods=['POST'])
@cross_origin()
